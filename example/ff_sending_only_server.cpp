    ...
    for (i = 0; i < nevents; ++i) {
        ...
        if (events[i].data.fd == sockfd) {
            while (1) {
                ...
                //  the following doesn't work, F-stack failed to write after 32 packages.
                // no matter it's 512 or 1024 bytes each time.
                for (int k = 0; k < TEST_TIMES; ++k, ++out.seq) {
                    ff_write( events[i].data.fd, &out, sizeof(out));
                }
